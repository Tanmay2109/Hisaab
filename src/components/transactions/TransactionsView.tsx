...
interface TransactionsViewProps {
  ...
  accounts: Account[];
  onAddTransaction: (data: Omit<Transaction, "id">) => Promise<void | boolean>;
  onDeleteTransaction: (
...
export const TransactionsView: React.FC<TransactionsViewProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Transactions Table (Desktop) & Mobile Cards */}
        ...
        <div className="block md:hidden divide-y divide-slate-100 dark:divide-slate-800">
            ...
            <div key={t.id} className="p-3.5 flex items-center justify-between gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="inline-flex items-center rounded-md bg-slate-100 px-1.5 py-0.5 text-[9px] font-bold text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                    {t.category}
                  </span>
          ...
          {filteredTransactions.length === 0 && (
            <div className="py-10 text-center text-slate-400 px-4">
              <Receipt className="mx-auto h-8 w-8 mb-2 stroke-1" />
              <p className="text-xs font-medium">No transactions match the selected filters.</p>
            </div>
          )}
        </div>
      </div>
      ...
      {isOpenAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs">
          <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-3xl bg-white p-5 sm:p-6 shadow-2xl dark:bg-slate-900 border border-slate-100 dark:border-slate-800">
      ...
      {showReceiptModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs">
          <div className="w-full max-w-md max-h-[90vh] overflow-y-auto rounded-3xl bg-white p-5 sm:p-6 shadow-2xl dark:bg-slate-900 border border-slate-100 dark:border-slate-800">
            <div className="flex items-center justify-between mb-4">
