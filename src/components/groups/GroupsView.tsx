      ...
      {groups.length > 0 ? (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-4">
          ...
          <div className="space-y-2 lg:col-span-1">
            ...
            <div className="flex lg:flex-col overflow-x-auto lg:overflow-x-visible pb-2 lg:pb-0 gap-2 lg:gap-1.5 flex-nowrap scrollbar-none">
              {groups.map((g) => (
                <button
                  key={g.id}
                  onClick={() => setSelectedGroupId(g.id)}
                  className={`shrink-0 lg:shrink lg:w-full text-left rounded-xl px-3 py-2.5 transition ${
                    g.id === selectedGroup?.id
                      ? 'bg-[#5A5A40] text-white shadow-xs'
                      : 'bg-white text-[#33332d] border border-[#e2e2d8] hover:bg-[#fafaf6] dark:bg-[#242420] dark:border-[#33332c] dark:text-[#e5e5dc]'
                  }`}
                >
                  <p className="text-xs font-bold whitespace-nowrap lg:whitespace-normal">{g.name}</p>
                  <p
                    className={`text-[10px] mt-0.5 whitespace-nowrap lg:whitespace-normal ${
